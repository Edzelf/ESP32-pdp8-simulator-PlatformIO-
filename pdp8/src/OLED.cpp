             ...
             int bl_pin )
{
  ...
  Wire.begin ( sda_pin, scl_pin, (uint32_t)400000 ) ;	          // Init I2C
	dsp_ok = i2cScan() ;						                              // Test I2C bus
  ssdbuf = (page_struct*) malloc ( 8 * sizeof(page_struct) ) ;	// Create buffer for screen
  clear() ;							                                        // Clear the display
  if ( bl_pin >= 0 )                                            // Backlight to be switched?
  {
    pinMode ( bl_pin, OUTPUT ) ;                                // Yes, set pin to output
    digitalWrite ( bl_pin, HIGH ) ;                             // Activate backlight
  }
}
...
void OLED::display ( bool force )
{
  ...
  if ( ! dsp_ok )                                             // Display found?
  {
    return ;                                                  // No, leave without action
  }
